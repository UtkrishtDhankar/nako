...
inline static void alloc_message(unsigned char **message, const uint64_t orig_msg_len)
{
	...
	/* If we don't have enough space for the 0x80 + message length padding,
	   make more space */
	if (64 - orig_msg_len % 64 < 9)
		final_len += 64;

	*message = malloc(final_len * sizeof(**message));
...
char *sha1(const char *file_name)
{
	...
	alloc_message(&message, message_size_bytes);
